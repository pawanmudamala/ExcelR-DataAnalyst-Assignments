...
SELECT * FROM Payments WHERE amount BETWEEN 50000 AND 100000;


SELECT 
    MONTHNAME(paymentDate) AS Month,
    CONCAT(FLOOR(SUM(Amount) / 1000), 'K') AS Formatted_Amount
FROM 
    payments
GROUP BY 
    MONTHNAME(paymentDate)
HAVING 
    SUM(Amount) BETWEEN 50000 AND 1000000
ORDER BY 
    Formatted_Amount DESC;
...
INSERT INTO Laptop (Laptop_ID, Laptop_Name) VALUES
(1, 'Laptop A'),
(2, 'Laptop B'),
(3, 'Laptop C');
...
-- Query to find names of employees and their related managers
SELECT 
...
-- Create facility table with fields and primary key auto-increment
...
-- Update the Name column to remove spaces
UPDATE University
...
SELECT 
    ...
    COUNT(OrderNumber) AS Total_Products_Sold,
    ROUND((COUNT(OrderNumber) / (SELECT COUNT(*) FROM orders WHERE YEAR(orderdate) = Year) * 100), 2) AS Percentage_of_Total_Value
FROM 
    orders
GROUP BY 
    Year;
...
BEGIN
    SELECT 
        ...
        country,
        CONCAT(FORMAT(SUM(amount), 0), 'K') AS TotalAmount
    FROM 
        Payments
    JOIN 
...
SELECT
    ...
    COUNT(*) AS OrderCount,
    CONCAT(FORMAT((COUNT(*) - LAG(COUNT(*), 12) OVER (ORDER BY YEAR(orderdate), MONTH(orderdate))) / NULLIF(LAG(COUNT(*), 12) OVER (ORDER BY YEAR(orderdate), MONTH(orderdate)), 0) * 100, 0), '%') AS YoY_PercentageChange
FROM
    Orders
GROUP BY
    Year, Month, YEAR(orderdate), MONTH(orderdate)
ORDER BY
...
BEGIN
    ...
    -- Calculate years and months difference from DOB to current date
    SELECT 
        TIMESTAMPDIFF(YEAR, dob, CURDATE()) INTO years;
    SELECT 
        TIMESTAMPDIFF(MONTH, dob, CURDATE()) % 12 INTO months;
...
SELECT 
    ...
    COALESCE(C.customerName, O.customerName) AS customerName,
    COUNT(O.orderNumber) AS orderCount
FROM 
    (
        SELECT customerNumber, NULL AS customerName, orderNumber
        FROM Orders
        UNION ALL
        SELECT NULL, customerName, NULL
        FROM Customers
    ) AS O
LEFT JOIN 
    Customers C ON C.customerNumber = O.customerNumber
GROUP BY 
    COALESCE(C.customerNumber, O.customerNumber),
...
-- Create the stored procedure Insert_Emp_EH
DELIMITER //
